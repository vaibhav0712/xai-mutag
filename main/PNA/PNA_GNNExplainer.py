from PNA_Train import PNA_Molecule
...
def initialize_xai_pipeline(model_checkpoint_path, dataset_path, rdf_path):
    ...
    # Wrap for explainer to prevent RGCN crash
    wrapper_model = base_model
...
def generate_explanation_payload(molecule_uri_str, pipeline_resources):
    ...
    if explanation.edge_mask is not None:
        # edge_mask is already 1D, so we remove the .sum(dim=1)
        raw_edge_scores = explanation.edge_mask
        
        # Fixed variable naming to avoid mixing up node and edge maximums
        max_edge_score = raw_edge_scores.max().item() if raw_edge_scores.max().item() > 0 else 1.0
        edge_scores = (raw_edge_scores / max_edge_score).cpu().tolist()
    else:
        # Fixed data.edges -> data.num_edges
        edge_scores = [0.0] * data.num_edges
    ...
    for i in range(data.edge_index.size(1)):
        ...
        if src < dst: 
            edges_payload.append({
                "source": src,
                "target": dst,
                "importance_score":round(edge_scores[i], 4),
                "type": bond_decoder[data.edge_type[i].item()] # Fixed decoder usage
            })
    return json.dumps({
...
if __name__ == "__main__":
    ...
    MODEL_PATH = models_dir / 'pna_model_checkpoint.pt'
    ...
    TEST_URI = "http://dl-learner.org/carcinogenesis#d152" # 152
